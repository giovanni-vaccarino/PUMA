#!/usr/bin/env python3
"""
VL Baseline: NoThinking — disable reasoning, direct answer only.

Prefills the thinking block with a closing statement, forcing the model
to skip reasoning and answer directly.

Usage:
    python baselines_vl/run_nothinking_vl.py \
        --model Qwen/Qwen3-VL-8B-Thinking \
        --dataset mathvista \
        --benchmark experiments_mdh/benchmark_vl/mathvista_test.jsonl \
        --output-dir experiments_mdh/data_vl/baselines/nothinking/mathvista
"""
...
# No-thinking prefill: close the think block immediately
NOTHINKING_PREFILL = "<think>\nOkay, I think I have finished thinking.\n</think>\n\n"


def main():
    parser = argparse.ArgumentParser(description="VL Baseline: NoThinking")
    ...
    # Build prompts with no-thinking prefill
    vllm_inputs = []
    for q in tqdm(questions, desc="Building NoThinking prompts"):
        base = build_base_prompt_vl(
            processor, args.model, q["question"], q["image_path"],
            task_type, "default",
        )
        prompt = base + NOTHINKING_PREFILL
        img = load_image(q["image_path"])
        vllm_inputs.append({"prompt": prompt, "multi_modal_data": {"image": img}})

    print(f"Generating {len(vllm_inputs)} NoThinking answers...")
    ...
    out_path = os.path.join(args.output_dir, "nothinking_results.json")
