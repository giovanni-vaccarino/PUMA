#!/bin/bash
# =============================================================================
# PUMA-VL offline pipeline (config-driven) — vision-language variant.
#
# Mirrors run_pipeline.sh, but for multimodal (image + text) reasoning models.
# The modality-independent stages are REUSED from puma/ (step segmentation,
# redundancy detection, two-stage stop decision); the generation and statistics
# stages use the puma_vl/ variants that carry the image through vLLM.
#
#   Step 1a  Generate full-CoT answers          (puma_vl/run_vllm_vl.py)     [optional]
#   Step 1b  Segment reasoning into steps        (puma/separate_steps.py)
#   Step 1.5 Redundancy Detector filtering       (puma/embed_and_filter_steps.py)
#   Step 2   Trial answers + confidence          (puma_vl/gen_trial_answers_vl.py)
#   Step 3   Two-stage stopping decision         (puma/extract_final_candidates.py)
#   Step 4   Regenerate from truncated prefix     (puma_vl/gen_prefixed_answers_vl.py)
#   Step 5   Accuracy / token-reduction stats     (puma_vl/statistics_puma_vl.py)
#
# Usage:
#   bash run_pipeline_vl.sh <config> <base_dir> <model> <dataset> [benchmark.jsonl]
#
#   <config>     a file under configs/, e.g. configs/DS-7B.conf (the same configs
#                the text pipeline uses; only the detection/RD knobs are read)
#   <base_dir>   working directory; all intermediate + output files go here
#   <model>      HF VL model name/path (e.g. Qwen/Qwen3-VL-8B-Thinking, moonshotai/Kimi-VL-A3B-Thinking)
#   <dataset>    VL dataset name (mathvista | mathvision | mmmu-pro ...)
#   [benchmark]  optional JSONL with {question, answer, image ...}. If given and
#                <base_dir>/answers.json does NOT exist, Step 1a generates it.
#                Build VL benchmarks with:  python puma_vl/dataset_utils.py --dataset <dataset>
#
# Example:
#   bash run_pipeline_vl.sh configs/DS-7B.conf runs/qwenvl8b_mathvista \
#        Qwen/Qwen3-VL-8B-Thinking mathvista data/benchmark_vl/mathvista_test.jsonl
#
# Multi-GPU: tensor-parallel size is auto-detected from CUDA_VISIBLE_DEVICES.
# =============================================================================
set -euo pipefail

CONFIG="${1:?Usage: run_pipeline_vl.sh <config> <base_dir> <model> <dataset> [benchmark.jsonl]}"
BASE_DIR="${2:?Usage: run_pipeline_vl.sh <config> <base_dir> <model> <dataset> [benchmark.jsonl]}"
MODEL="${3:?Usage: run_pipeline_vl.sh <config> <base_dir> <model> <dataset> [benchmark.jsonl]}"
DATASET="${4:?Usage: run_pipeline_vl.sh <config> <base_dir> <model> <dataset> [benchmark.jsonl]}"
BENCHMARK_FILE="${5:-}"

# Run from the repository root so `python puma_vl/<stage>.py` / `python puma/<stage>.py`
# imports resolve regardless of the caller's working directory.
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$REPO_ROOT"
PY="${PYTHON:-python}"

# ---- Load config -----------------------------------------------------------
# shellcheck source=/dev/null
source "$CONFIG"
mkdir -p "$BASE_DIR"

# Defaults for knobs not set in the config (match the reference pipeline).
LOOP_BREAKER_SIMILARITY_THRESHOLD="${LOOP_BREAKER_SIMILARITY_THRESHOLD:-${SIMILARITY_THRESHOLD}}"
PROMPT_VERSION="${PROMPT_VERSION:-default}"
CONFIDENCE_MODE="${CONFIDENCE_MODE:-token_in_boxed}"
CONFIDENCE_AGGREGATION="${CONFIDENCE_AGGREGATION:-geometric}"
ALWAYS_CHECK_FIRST_N="${ALWAYS_CHECK_FIRST_N:-1}"
EMBEDDING_WINDOW_SIZE="${EMBEDDING_WINDOW_SIZE:-1}"
TRIGGER_MODE="${TRIGGER_MODE:-previous}"
MIN_STOP_STEP="${MIN_STOP_STEP:-10}"
ENABLE_EMBEDDING_FILTER="${ENABLE_EMBEDDING_FILTER:-true}"
ANSWER_GEN_MAX_TOKENS="${ANSWER_GEN_MAX_TOKENS:-32768}"
ANSWER_FIX_MAX_TOKENS="${ANSWER_FIX_MAX_TOKENS:-2048}"
MAX_TRIAL_TOKENS="${MAX_TRIAL_TOKENS:-30}"
TEMPERATURE="${TEMPERATURE:-0.6}"
TOP_P="${TOP_P:-0.95}"
TRIAL_DECODING="${TRIAL_DECODING:-sampling}"
MAX_ANSWER_TOKENS="${MAX_ANSWER_TOKENS:-4096}"

# Tensor parallel size: auto-detect from visible GPUs.
if [ -n "${CUDA_VISIBLE_DEVICES:-}" ]; then
    TENSOR_PARALLEL_SIZE=$(echo "$CUDA_VISIBLE_DEVICES" | tr ',' '\n' | grep -c .)
else
    TENSOR_PARALLEL_SIZE=1
fi

# Optional argument fragments.
SEED_ARG=""; [ -n "${SEED:-}" ] && SEED_ARG="--seed ${SEED}"

# Output files (flat, inside base_dir).
ANSWERS="${BASE_DIR}/answers.json"
STEPS="${BASE_DIR}/steps.json"
FILTERED="${BASE_DIR}/filtered_steps.json"
TRIAL="${BASE_DIR}/trial_answers.json"
CANDIDATES="${BASE_DIR}/final_candidates.json"
PREFIXED="${BASE_DIR}/prefixed_answers.json"
STATS="${BASE_DIR}/statistics.txt"

echo "=================================================================="
echo "PUMA-VL pipeline | model=$MODEL | dataset=$DATASET"
echo "config=$CONFIG | base_dir=$BASE_DIR | TP=$TENSOR_PARALLEL_SIZE"
echo "embedding_filter=$ENABLE_EMBEDDING_FILTER | m=$CONSECUTIVE_REDUNDANCY_STOP"
echo "=================================================================="

# ---- Step 1a: generate full-CoT answers (optional) -------------------------
if [ -f "$ANSWERS" ]; then
    echo ">>> Step 1a: skipped (answers.json already exists)"
elif [ -n "$BENCHMARK_FILE" ]; then
    echo ">>> Step 1a: generating VL answers from $BENCHMARK_FILE"
    # shellcheck disable=SC2086
    "$PY" puma_vl/run_vllm_vl.py \
        --dataset "$DATASET" \
        --model "$MODEL" \
        --dataset_path "$BENCHMARK_FILE" \
        --output_path "$ANSWERS" \
        --max_tokens "$ANSWER_GEN_MAX_TOKENS" \
        --answer_max_tokens "$ANSWER_FIX_MAX_TOKENS" \
        --prompt-version "$PROMPT_VERSION" \
        $SEED_ARG
else
    echo "ERROR: $ANSWERS not found and no benchmark file given."
    echo "       Provide a 5th argument (a VL benchmark JSONL) to generate answers,"
    echo "       or place a pre-generated answers.json there. Build VL benchmarks with:"
    echo "         python puma_vl/dataset_utils.py --dataset $DATASET"
    exit 1
fi

# ---- Step 1b: segment reasoning into steps (reused from puma/) --------------
echo ">>> Step 1b: separating reasoning steps"
"$PY" puma/separate_steps.py --input_json "$ANSWERS" --output_json "$STEPS" --workers 0

# ---- Step 1.5: Redundancy Detector filtering (reused from puma/) ------------
CURRENT_STEPS="$STEPS"
TRIAL_ARGS=""
if [ "$ENABLE_EMBEDDING_FILTER" = "true" ]; then
    echo ">>> Step 1.5: redundancy-detector filtering"
    "$PY" puma/embed_and_filter_steps.py \
        --input-file "$STEPS" \
        --output-file "$FILTERED" \
        --embedding-model "$EMBEDDING_MODEL" \
        --similarity-threshold "$SIMILARITY_THRESHOLD" \
        --always-check-first-n "$ALWAYS_CHECK_FIRST_N" \
        --window-size "$EMBEDDING_WINDOW_SIZE" \
        --trigger-mode "$TRIGGER_MODE" \
        --consecutive-redundancy-stop "$CONSECUTIVE_REDUNDANCY_STOP" \
        --consecutive-redundancy-min-step "$CONSECUTIVE_REDUNDANCY_MIN_STEP" \
        --loop-breaker-similarity-threshold "$LOOP_BREAKER_SIMILARITY_THRESHOLD"
    CURRENT_STEPS="$FILTERED"
    TRIAL_ARGS="--respect-embedding-filter"
else
    echo ">>> Step 1.5: skipped (embedding filter disabled)"
fi

# ---- Step 2: VL trial answers + confidence ---------------------------------
echo ">>> Step 2: generating VL trial answers"
# shellcheck disable=SC2086
"$PY" puma_vl/gen_trial_answers_vl.py \
    --questions-file "$CURRENT_STEPS" \
    --output-file "$TRIAL" \
    --model "$MODEL" \
    --max-tokens "$MAX_TRIAL_TOKENS" \
    --temperature "$TEMPERATURE" \
    --top_p "$TOP_P" \
    --tensor-parallel-size "$TENSOR_PARALLEL_SIZE" \
    --dataset "$DATASET" \
    --trial-decoding "$TRIAL_DECODING" \
    --confidence-mode "$CONFIDENCE_MODE" \
    --confidence-aggregation "$CONFIDENCE_AGGREGATION" \
    --prompt-version "$PROMPT_VERSION" \
    $TRIAL_ARGS $SEED_ARG

# ---- Step 3: two-stage stopping decision (reused from puma/) ----------------
echo ">>> Step 3: extracting final candidates"
FILTERED_STEPS_ARG=""
if [ "$ENABLE_EMBEDDING_FILTER" = "true" ]; then
    FILTERED_STEPS_ARG="--filtered-steps $FILTERED --forced-stop-min-confidence $FORCED_STOP_MIN_CONFIDENCE"
fi
# shellcheck disable=SC2086
"$PY" puma/extract_final_candidates.py \
    --input-file "$TRIAL" \
    --output-file "$CANDIDATES" \
    --confidence-threshold "$CONFIDENCE_THRESHOLD" \
    --epsilon "$EPSILON" \
    --consecutive "$CONSECUTIVE" \
    --min-stop-step "$MIN_STOP_STEP" \
    --dataset "$DATASET" \
    --workers 0 \
    $FILTERED_STEPS_ARG

# ---- Step 4: regenerate from truncated prefix ------------------------------
echo ">>> Step 4: generating VL prefixed answers"
# shellcheck disable=SC2086
"$PY" puma_vl/gen_prefixed_answers_vl.py \
    --final-candidates "$CANDIDATES" \
    --questions-file "$CURRENT_STEPS" \
    --output-file "$PREFIXED" \
    --model "$MODEL" \
    --max-tokens "$MAX_ANSWER_TOKENS" \
    --tensor-parallel-size "$TENSOR_PARALLEL_SIZE" \
    --dataset "$DATASET" \
    --prompt-version "$PROMPT_VERSION" \
    --confidence-aggregation "$CONFIDENCE_AGGREGATION" \
    $SEED_ARG

# ---- Step 5: statistics ----------------------------------------------------
echo ">>> Step 5: computing VL statistics"
STATS_EMBEDDING_ARGS=""
if [ "$ENABLE_EMBEDDING_FILTER" = "true" ]; then
    STATS_EMBEDDING_ARGS="--enable-embedding-filter --trial-answers $TRIAL"
fi
# shellcheck disable=SC2086
"$PY" puma_vl/statistics_puma_vl.py \
    --original "$ANSWERS" \
    --compressed "$PREFIXED" \
    --model "$MODEL" \
    --dataset "$DATASET" \
    --confidence-threshold "$CONFIDENCE_THRESHOLD" \
    --epsilon "$EPSILON" \
    --consecutive "$CONSECUTIVE" \
    --output "$STATS" \
    --workers 0 \
    $STATS_EMBEDDING_ARGS

echo "=================================================================="
echo "PUMA-VL pipeline complete. Outputs in: $BASE_DIR"
echo "Statistics: $STATS"
echo "=================================================================="
