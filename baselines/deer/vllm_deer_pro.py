...
def parse_args():
    ...
    parser.add_argument('--alpha', type=float, default=1.0, help="Fluctuation penalty strength for DEER-Pro (default: 1.0)")
    ...
    parser.add_argument('--output_path', type=str, default='./outputs/deer_pro.jsonl') # output path
...
def main():
    ...
    print(f"DEER-Pro alpha (fluctuation penalty): {args.alpha}")
    print(f"DEER-Pro N (number of inducing prompts): 4")
    ...
    # DEER-Pro: 4 answer inducing prompts for calibrated confidence
    answer_prompt_strs = [
        "\n**Final Answer**\n\\boxed",
    ...
    num_inducing_prompts = len(answer_prompt_strs)
    ...
    for i, question_data in enumerate(questions_json):
        ...
        if args.no_thinking == 1:
            ...
            'prob_check_confidences': [],  # DEER-Pro: collect N confidences
            'prob_check_collected': 0,     # DEER-Pro: count collected results
            'output_dict': {}, 
            'error_message': None, 
            'question_index': i 
        }
    ...
    while active_questions_indices: #  indexes [0,1,2,...,n]
        ...
        for q_idx in current_active_indices_for_batching:
            ...
            if state['state'] in ['finished', 'error', 'waiting_prob_checks']:
                 continue
            ...
            try:
                ...
                # Check state — DEER-Pro: submit N answer inducing prompts in parallel
                elif state['state'] == 'needs_prob_check':
                    # Reset collection state for this round
                    state['prob_check_confidences'] = []
                    state['prob_check_collected'] = 0

                    # Check context window for the longest prompt
                    longest_prompt = state['current_full_sequence'] + max(answer_prompt_strs, key=len)
                    longest_prompt_len = len(tokenizer.encode(longest_prompt, add_special_tokens=False))
                    required_space = longest_prompt_len + args.prob_check_max_tokens

                    if required_space > args.model_context_len:
                         state['state'] = 'error'
                         state['error_message'] = f"Probability check generation prompt exceeds context window ({args.model_context_len}). Estimated length: {required_space}"
                         state['output_dict'] = {'question': state['question_data']['question'], 'generated_text': state['generated_thinking_history'] + state['generated_answer_history'] + "\n" + state['error_message'], 'gold_answer': state['question_data']['answer']}
                         print(f"\nQuestion {q_idx}: {state['error_message']}")
                         if q_idx in active_questions_indices:
                            active_questions_indices.remove(q_idx)
                    ...
                    # Add N prompts to batch (one per answer inducing prompt)
                    prob_check_sampling_params = SamplingParams(
                        max_tokens=args.prob_check_max_tokens,
                        stop=pred_prob_stop_tokens,
                        logprobs=1,
                    )
                    for p_idx in range(num_inducing_prompts):
                        prompt_for_prob_check = state['current_full_sequence'] + answer_prompt_strs[p_idx]
                        batch_prompts.append(prompt_for_prob_check)
                        batch_sampling_params.append(prob_check_sampling_params)
                        batch_request_info.append((q_idx, 'prob_check_gen'))
                        current_batch_count += 1

                    state['state'] = 'waiting_prob_checks'
                    continue  # Skip the single-prompt append below
        ...
        for i, output in enumerate(batch_outputs):
            ...
            try:
                ...
                if step_type == 'think':
                    ...
                    # DEER-Pro: collect confidence from each inducing prompt
                    if completion_output.logprobs:
                         conf = calculate_average_max_prob_from_logprobs(completion_output.logprobs, args.policy)
                    else:
                         print(f"Warning: No logprobs returned for prob_check_gen for question {q_idx}. Setting conf to 0.0.")
                         conf = 0.0

                    state['prob_check_confidences'].append(conf)
                    state['prob_check_collected'] += 1

                    # Accumulate trial answer tokens for CRT calculation
                    state['num_trial_answer_tokens'] += len(completion_output.token_ids)

                    # Only make decision when all N prompts are collected
                    if state['prob_check_collected'] >= num_inducing_prompts:
                        # Compute DEER-Pro calibrated confidence: Ccali = Cavg - α * CMAD
                        confidences = state['prob_check_confidences']
                        c_avg = sum(confidences) / len(confidences)
                        c_mad = sum(abs(c - c_avg) for c in confidences) / len(confidences)
                        c_cali = c_avg - args.alpha * c_mad

                        state['pred_prob'] = c_cali
                        print(f"\nQuestion {q_idx}: DEER-Pro confidences={[f'{c:.4f}' for c in confidences]}, "
                              f"Cavg={c_avg:.4f}, CMAD={c_mad:.4f}, Ccali={c_cali:.4f}")
                        ...
                        if c_cali > args.threshold or thinking_limit_reached:
                             state['state'] = 'needs_answer'
                             if thinking_limit_reached:
                                 print(f"  → Reached thinking limit ({current_generated_thinking_tokens}/{think_limit_tokens}). Switching to answer phase.")
                                 state['too_long'] = 1
                             else:
                                 print(f"  → Reached early exit threshold ({c_cali:.4f} > {args.threshold}). Switching to answer phase.")
                                 state['high_prob'] = 1
                        else:
                             ...
                             print(f"  → Threshold not reached ({c_cali:.4f} <= {args.threshold}). Continuing thinking.")
